...
//饼图
export function pieFun(type,config,data){
    ...
    switch(groupFun){
        case "comPie":
            resOption = comPie(comOptoin,barConfig,data)
            break;
        case "picPie":
            resOption = picPie(comOptoin,barConfig,data)
            break;
    }
...
//常规的饼图
export function comPie(comOptoin,config,data){
    let colorList = config.colorConfig.colorList;
    let option = Object.assign(comOptoin, {});
    let resData = objToArr(data).yData||[
        {'name': '北京', 'value': 5600},
        {'name': '上海','value': 3600},
        {'name': '广州','value': 1500}, 
        {'name': '深圳','value': 2000}, 
    ...
    option.tooltip = {
        textStyle: {
            color: config.colorConfig.tootipColor,
            fontSize: 12
        },
...
//有图片装饰的饼图
export function picPie (comOptoin,config,data){
  let color = config.colorConfig.colorList;
  let option = Object.assign(comOptoin, {});
  ...
  let xData =  objToArr(data).xData||['火车', '飞机', '客车', '轮渡','动车'];
  let  resData = objToArr(data).yData ||[
      {name: '火车', value: 20}, {name: '飞机', value: 10 }, 
      {name: '客车',value: 30 }, {name: '轮渡',value: 40}, {name: '动车',value: 40}
   ];
    let targetData = [];
    resData.map((o,idx)=>{
        targetData.push({
        ...
        itemStyle: {
            normal: {
                ...
                labelLine: {
                    ...
                    length2: 100,
                    show: true,
                    color: config.colorConfig.seriseLableLineColor
                }
            }
        },
        data: targetData
    ...
    option.legend = {
       ...
       // bottom: '5%',
        //align: 'right',
        textStyle: {
            color: config.colorConfig.legendColor
        },
        itemGap: 20
    }
 ...
 function objToArr(data){
    ...
    data[0].data.map((o)=>{
        resObj.xData.push(o.name)
    });
    resObj.yData = data[0].data
    return resObj
}